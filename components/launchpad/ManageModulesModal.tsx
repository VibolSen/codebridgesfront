...
import { Sliders, PowerOff, Sparkles, User, Store } from 'lucide-react';
import { SystemModule, CATALOG_MODULES } from './types';
      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            animate="visible"
            exit="exit"
            className={`${cardStyles.base} max-w-xl w-full p-6 sm:p-7 shadow-2xl text-slate-800 space-y-5 relative max-h-[90vh] overflow-y-auto`}
          >
            {/* Close Button */}
            <button
              onClick={onClose}
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-xl font-extrabold text-slate-900">
                  {selectedModule ? `Configure ${selectedModule.title}` : 'Configure Modules for Organization'}
                </h3>
            ...
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-40 overflow-y-auto pr-1">
                {userOrganizations.map((org) => {
                  ...
                  const Icon = org.type === 'Personal' ? User : org.type === 'Outlet' ? Store : AppIcons.Organization;
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center justify-between p-2.5 rounded-xl border transition-all text-left cursor-pointer ${
                        isSelected
                          ? 'border-brand bg-brand-subtle shadow-2xs'
                          : 'border-slate-200 hover:border-slate-300 bg-white'
                      }`}
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        <Icon className={`w-4 h-4 shrink-0 ${isSelected ? 'text-brand' : 'text-slate-500'}`} />
                        <div className="min-w-0">
                          <p className="font-extrabold text-xs text-slate-900 truncate">{org.name}</p>
                          <span className="text-[9px] text-slate-400 font-semibold uppercase">{org.type}</span>
                        </div>
                      </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          >
                            {mod.logoUrl ? (
                              <img src={mod.logoUrl} alt={mod.name} className="w-full h-full object-contain" />
                            ) : (
                              <Icon className="w-4 h-4" />
                            )}
                          </div>
                          <div className="min-w-0">
                            <p className="text-xs font-bold text-slate-900 truncate">{mod.name}</p>
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          >
                            {isChecked && <AppIcons.Check className="w-3.5 h-3.5 stroke-[3]" />}
                          </div>
                        </div>
                      </div>
                    );
              ...
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-xs space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-slate-900 font-extrabold text-sm">
                    <selectedModule.icon className="w-5 h-5 text-brand" />
                    <span>{selectedModule.title}</span>
                  </div>
                  <span
                    className={`text-[10px] font-extrabold px-2 py-0.5 rounded-full ${
                      isSingleModuleEnabled ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600'
                    }`}
                  >
                    Currently: {isSingleModuleEnabled ? 'ENABLED' : 'DISABLED'}
                  </span>
                </div>
                <p className="text-[11px] text-slate-600 leading-relaxed">{selectedModule.description}</p>
              </div>
            )}
            ...
            <div className="space-y-2 pt-2 border-t border-slate-100">
              {selectedModule ? (
                <div className="flex flex-col sm:flex-row gap-2">
                  {isSingleModuleEnabled ? (
                    <button
                      ...
                      className="flex-1 py-3 rounded-2xl bg-rose-600 hover:bg-rose-700 text-white font-extrabold text-xs shadow-md transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                    >
                ...
                <div className="space-y-2">
                  <button
                    onClick={onSaveModalChanges}
                    className={`w-full py-3.5 ${buttonStyles.primary}`}
                  >
                    <AppIcons.Check className="w-4 h-4 stroke-[3]" />
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="flex-1 py-2 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 font-bold text-xs transition-colors flex items-center justify-center gap-1 cursor-pointer"
                    >
