...
export function PlanTierSelector({
  ...
  return (
    <div className="space-y-12">
      ...
      >
        <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-brand-subtle border border-brand-border/60 text-brand text-[10px] font-bold tracking-wider uppercase shadow-xs">
      ...
      >
        {plans.map((plan, i) => {
          ...
          return (
            ...
            >
              {plan.badge && (
                <span className="absolute -top-3 right-6 px-3 py-1 bg-gradient-to-r from-brand to-brand-hover text-white text-[10px] font-black uppercase tracking-wider rounded-full shadow-md shadow-brand/20">
                  {plan.badge}
                </span>
              )}
              ...
              <motion.button
                ...
                className={`w-full py-3 rounded-xl font-bold text-xs transition-all flex items-center justify-center gap-2 cursor-pointer ${
                  isSelected
                    ? `bg-gradient-to-r ${plan.gradient} text-white shadow-lg shadow-brand/20`
                    : 'bg-slate-100 hover:bg-brand-subtle hover:text-brand border border-slate-200 text-slate-800'
                }`}
              >
                <span>{plan.cta}</span>
      ...
      <div className="flex flex-wrap justify-center gap-4 text-slate-600 text-xs font-semibold">
        ...
        <span className="flex items-center gap-1.5 bg-white border border-slate-200/80 px-3.5 py-2 rounded-xl shadow-xs">
          <BarChart3 className="w-4 h-4 text-brand" /> Real-Time Analytics
        </span>
      </div>
    </div>
  );
