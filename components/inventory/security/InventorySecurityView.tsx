...
import { getUsersApi, verifyPinApi } from '@/lib/api';
import { PosAccessStaffTable, PosAccessKeypadTester, PosAccessPinModal } from '@/components/pos/access';
import { InventoryRbacCard } from '@/components/inventory';
...
export function InventorySecurityView() {
    ...
    <div className="space-y-6 max-w-7xl mx-auto font-sans text-slate-900 pb-16">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-200/80 shadow-[0_2px_12px_rgba(15,23,42,0.03)]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-900 tracking-tight">Cashier PINs &amp; Security</h1>
            <p className="text-xs text-slate-500 font-medium">4-digit register PIN credentials, supervisor overrides &amp; drawer security matrix</p>
          </div>
        </div>
      ...
      {/* 2. Inventory RBAC Overview Card */}
      <InventoryRbacCard />

      {/* 3. Main Staff Access Matrix & Keypad Tester */}
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-2xs flex flex-wrap items-center justify-between gap-3">
            ...
            <select
              ...
              className="px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-200 text-xs font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-brand/30 focus:border-brand"
            >
