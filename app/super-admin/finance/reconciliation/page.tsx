...
export default function AdminReconciliationPage() {
  ...
  // Selected Exception for Resolution Modal
    ...
    <div className="p-8 space-y-6 max-w-7xl mx-auto font-sans text-slate-800">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="px-5 py-2.5 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-bold text-xs shadow-md shadow-orange-500/30 transition-all flex items-center gap-2 disabled:opacity-50"
        >
                    ...
                    <td className="py-3.5 px-4 text-right">
                      {ex.status === 'pending' ? (
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-slate-900 hover:bg-black text-white text-xs font-bold transition-all shadow-xs"
                        >
                          Resolve Discrepancy
                        </button>
                      ) : (
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-white rounded-2xl p-6 shadow-2xl space-y-4 border border-slate-200">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              ...
              <button onClick={() => setSelectedException(null)} className="text-slate-400 hover:text-slate-600">
                <X className="w-5 h-5" />
              </button>
            </div>

            <div className="p-3 bg-slate-50 rounded-xl text-xs space-y-1 font-mono">
              <p><span className="text-slate-500">Ref:</span> <span className="font-bold text-slate-900">{selectedException.merchant_reference}</span></p>
              <p><span className="text-slate-500">Discrepancy:</span> <span className="font-extrabold text-rose-600">${Number(selectedException.discrepancy_amount).toFixed(2)}</span></p>
              <p><span className="text-slate-500">Type:</span> <span className="font-bold text-amber-700">{selectedException.exception_type}</span></p>
            </div>
            ...
            <form onSubmit={handleResolveSubmit} className="space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  type="submit"
                  disabled={resolving}
                  className="px-5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs shadow-md transition-colors disabled:opacity-50"
                >
