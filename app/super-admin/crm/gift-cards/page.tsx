...
import { Gift, Plus, RefreshCw, CreditCard } from 'lucide-react';
    ...
    <div className="p-8 space-y-6 max-w-7xl mx-auto font-sans text-slate-800">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">Issue digital gift cards, prepaid customer vouchers, and store credit cards</p>
        </div>
        <button
        ...
        <table className="w-full text-left text-xs">
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              ...
              <tr><td colSpan={4} className="py-8 text-center text-slate-400 font-semibold">No gift cards issued.</td></tr>
            ) : (
              cards.map((c) => (
                <tr key={c.id} className="hover:bg-slate-50/50">
                  ...
                  <td className="py-3.5 px-4 font-mono font-black text-pink-600 text-sm">${Number(c.balance).toFixed(2)}</td>
          ...
          <div className="w-full max-w-md bg-white rounded-2xl p-6 shadow-2xl space-y-4">
            ...
            <form onSubmit={handleCreateGiftCard} className="space-y-3 text-xs">
              ...
              <div className="flex gap-2 pt-2">
                <button type="button" onClick={() => setShowModal(false)} className="flex-1 py-2.5 rounded-xl bg-slate-100 font-bold text-slate-700">Cancel</button>
                <button type="submit" className="flex-1 py-2.5 rounded-xl bg-pink-500 font-bold text-white shadow-md">Confirm & Issue</button>
              </div>
            </form>
          </div>
        </div>
