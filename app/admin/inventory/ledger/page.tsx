    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      >
        <div>
          ...
          <p className="text-xs text-slate-500">Immutable audit trail log of every stock movement, sale, receiving, and adjustment</p>
        </div>
      </motion.div>
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
                {movements.map((log, idx) => {
                  const badge = getMovementBadge(log.movement_type, log.quantity);
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4">
                        <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold border tracking-wider uppercase ${badge.style}`}>
                          <IconComp className="w-3 h-3" />
                          {badge.label}
                        </span>
                      </td>

                      <td className={`py-3.5 px-4 text-right font-extrabold font-mono text-xs ${isPositive ? 'text-emerald-600' : 'text-rose-600'}`}>
